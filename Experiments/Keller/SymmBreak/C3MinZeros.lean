...
def toC3Zeros (tc : C3MinZeroSorted n s) : C3Zeros n s where
  toTwoCubes := tc.toTwoCubes
  c3_zeros_sorted := tc.c3_sorted
  c3_more_nonzero := by
    rintro j j_range ⟨c3_nz, c3_z⟩ row row_range X_nz
    have c3_le_X := tc.c3_min_zero (C3Zeros.X row (by omega)) ?i1_true ?high_bits_nz
    case i1_true =>
      simp [C3Zeros.X, bv_toNat]; rw [Bool.eq_iff_iff, Nat.testBit_three]
      simp; omega
    case high_bits_nz =>
      intro j' j'_range j'_eq_row
      replace j'_eq_row : j' = row := by
        simp [C3Zeros.X, bv_toNat, j'_range] at j'_eq_row
    ...
    let NZSet : Finset (Fin (n+2)) := { j' | 2 ≤ j'.val ∧ (tc.data (C3Zeros.X row (by omega)))[j'.val] ≠ 0}

    unfold C3MinZero.count at c3_le_X
    generalize def_ZeroSet :
      Finset.filter (fun j' : Fin (n+2) => 2 ≤ j'.val ∧ (tc.data (C3Zeros.X row _))[j'.val] = 0) _
        = ZeroSet at c3_le_X
