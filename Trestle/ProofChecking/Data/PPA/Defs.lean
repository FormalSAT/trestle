...
--import Trestle.Data.Array
import Trestle.ProofChecking.Data.Array
...
def setNegatedClauseUntil_loop (τ : PPA) (C : IClause) (gen : Nat) : PPA :=
  let rec loop (τ : PPA) (i : Nat) : PPA :=
    if h : i < C.size then
      loop (τ.setLitUntil (-C.get i h) gen) (i + 1)
    else
      τ
  loop τ 0
...
def setNegatedClauseFor_loop (τ : PPA) (C : IClause) (extraBumps : Nat) : PPA :=
  let rec loop (τ : PPA) (i : Nat) : PPA :=
    if h : i < C.size then
      loop (τ.setLitFor (-C.get i h) extraBumps) (i + 1)
    else
      τ
  loop τ 0
...
theorem setNegatedClause_loop_drop (τ : PPA) {C : List ILit} {i j : Nat} :
  ...
  | succ j ih =>
    ...
    rw [setNegatedClause_loop.loop]
    simp only [Array.size_toArray, hi, ↓reduceDIte, Array.get_eq_getElem]
    rw [ih]
