...
import Trestle.ProofChecking.Data.PPA.Defs
...
def idxToPropFun (τ : PPA) (i : Nat) : PropFun IVar :=
  τ.varToPropFun ⟨IVar.ofIndex i, succ_pos _⟩
  ...
  . split
    . next n hn h =>
      have : n ∈ τ.reset.assignment := by
        rw [Array.get?_eq_getElem?, Array.getElem?_eq_some_iff] at hn
...
theorem unitPropM.aux_cons (τ : PPA) (l : ILit) (ls : List ILit) (unit? : Option ILit) :
  ...
  simp only [Array.size_toArray, List.length_cons, List.foldlM_toArray', List.foldlM_cons]
  match hl : τ.litValue? l with
  | some true => rfl
  | some false => rfl
  | none =>
