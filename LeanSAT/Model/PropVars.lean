...
theorem semVars_biImpl (φ₁ φ₂ : PropFun ν) :
    (PropFun.biImpl φ₁ φ₂).semVars ⊆ φ₁.semVars ∪ φ₂.semVars := by
  rw [biImpl_eq_impls]
...
theorem equivalentOver_of_equivalent : equivalent φ₁ φ₂ → PropFun.equivalentOver X ⟦φ₁⟧ ⟦φ₂⟧ :=
  fun h => Quotient.sound h ▸ PropFun.equivalentOver_refl ⟦φ₁⟧

theorem semVars_eq_of_equivalent (φ₁ φ₂ : PropForm ν) : equivalent φ₁ φ₂ →
    PropFun.semVars ⟦φ₁⟧ = PropFun.semVars ⟦φ₂⟧ :=
