...
def toPropFun (l : ILit) : PropFun Var :=
  if l.polarity then .var l.var else (.var l.var)ᶜ

@[simp]
theorem mk_toPropForm (l : ILit) : ⟦l.toPropForm⟧ = l.toPropFun := by
...
@[simp]
theorem toPropFun_mkPos (x : Var) : (mkPos x).toPropFun = .var x := by
  simp [toPropFun]
...
theorem satisfies_iff {τ : PropAssignment Var} {l : ILit} :
    τ ⊨ l.toPropFun ↔ τ l.var = l.polarity := by
...
def toPropFun (φ : ICnf) : PropFun Var :=
  φ.data.foldr (init := ⊤) (fun l φ => l.toPropFun ⊓ φ)

@[simp]
theorem mk_toPropForm (φ : ICnf) : ⟦φ.toPropForm⟧ = φ.toPropFun := by
